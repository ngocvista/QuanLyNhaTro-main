...
namespace MotelManagement.Business.Service
{
    public class RoomService : IRoomService
    {
        ...
        public async Task<List<Room>> 
            advancedSearchRoomList(string nameRoom, int? roomTypeId, int? status, string price, string sizePerson, int page)
        {
            // Business 
        ...
        public async Task AddNewImageRoom(List<IFormFile> lstformfile , int Roomid)
        {
            try
            {
                foreach (var item in lstformfile)
                {
                    ...
                    image.Url = FileName;
                    image.RoomId = Roomid;
                    //await _unitOfWork.roomRepository.AddImageAsync(image);
                }
            }catch (Exception ex)
            {
